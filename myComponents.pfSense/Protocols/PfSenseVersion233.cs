...
namespace myComponents.pfSense.Protocols
{
    ...
    public class PfSenseVersion233 : IPfSenseProtocol
    {
        ...
        public string Execute(pfSense pfSenseServer, CookieContainer cookieJar)
        {
            ...
            {
                throw new ApplicationException("Impossible d'accéder à la page d'authentification");
            }
            ...
            // Check if the response is the backup page -- the page must contain the download button : <button class="btn btn-primary" type="submit" value="Download configuration as XML" name="download"><i class="fa fa-download icon-embed-btn"> </i>Download configuration as XML</button>
            string btnBackupDef = "<button class=\"btn btn-primary\" type=\"submit\" value=\"Download configuration as XML\" name=\"download\"><i class=\"fa fa-download icon-embed-btn\"> </i>Download configuration as XML</button>";
            if (!backupPageContents.Contains(btnBackupDef))
            {
                throw new ApplicationException("The page retrieve is not the backup page. Download button not found.");
            }
